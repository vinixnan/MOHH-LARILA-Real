...
public class choiceFunction {
    ...
    public int getMaxChoiceFunction(double[][] measureMetrix1, int order1,
            ...
            int numberOfMeasurements, int alpha, long[] elapsedTime) {
        ...
        long[] elapsedTimeArray = new long[elapsedTime.length];
        for (int i = 0; i < elapsedTime.length; i++) {
            double percent=(((double)elapsedTime[i])/((double)sum))*100.0;
            elapsedTimeArray[i]=(long) Math.ceil(percent);
        }
